...
import {
  setMarketingFeatureAction,
  type MarketingFeatureState,
} from "@/app/admin/marketing/actions";
import { Button } from "@/components/ui/button";

const initial: MarketingFeatureState = { ok: false, message: "" };

export function MarketingFeatureToggle({ enabled }: { enabled: boolean }) {
    ...
    >
      <div>
        ...
        <p className="mt-1 text-sm text-white/55">
          When off, the Business marketing nav link is hidden and the public
          marketing pages redirect home. Existing company dashboards stay
      ...
      <label className="flex cursor-pointer items-center gap-3 rounded-2xl border border-white/15 bg-[#0c0c0c] px-4 py-3 text-sm text-white has-checked:border-accent has-checked:bg-accent/10">
        ...
        />
        Show Business marketing on the website
      </label>
