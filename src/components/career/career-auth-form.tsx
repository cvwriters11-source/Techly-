...
const initial: CareerAuthState = { ok: false, message: "" };
...
export function CareerAuthForm({ mode }: { mode: "login" | "register" }) {
  const action = mode === "register" ? signUpCareer : signInCareer;
    ...
    <form action={formAction} className="space-y-5">
      ...
      <p className="text-center text-sm text-white/55">
        {mode === "register" ? (
          ...
          <>
            New candidate?{" "}
            <Link href="/career/register" className="text-accent">
              Register
            </Link>
          </>
        )}
